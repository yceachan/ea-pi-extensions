...
- Read `AGENTS.md`, especially the conventions (including the tests-are-not-golden-truth and Agent Notes-are-not-golden-truth doctrines), plus the repository's defensive-patterns and testing documentation.
- Skim the architecture documentation before judging anything under the package tree; simplifications that fight the service map or event taxonomy need extra evidence.
- Use the Agent Note tree and its rules to understand intentional architecture. The most relevant implemented examples are the recorded drops, shared-coordinator, and capability-seam decisions, plus any twin adapter / dual backend notes.
...
- Hand-rolled code reimplements what a well-maintained external package or a Node builtin at the engine floor already provides, and the swap would delete the implementation plus its dedicated tests (per the dependency policy).
...
- Protocol automation and human UI APIs: prompt settlement and teardown on the protocol side; transcript rendering and interaction state on the UI side.
- LLM/tools/system prompt: stream/generate APIs, assemblers, registries, tool schema defaults, presentation hooks.
- Tool execution: foreground/background split, job ownership, output spill files, executor methods.
...
Introducing a dependency is a valid simplification move, not a policy exception: the repository's dependency policy — prefer maintained packages or engine-floor builtins over hand-rolling — owns the bar. When surveying, ask of protocol parsers, framers, retry/backoff loops, glob matchers, diff engines, and similar infrastructure: does a well-maintained npm package or a Node builtin at the repo's engine floor already do this?
...
- Check the Agent Note tree first: vendored libraries, twin adapters, and other recorded seams are settled — a swap that collapses one needs to beat the recorded rationale, not just cite the policy.
...
- The idea is correct but tiny. Add a targeted TODO/FIXME/XXX instead, using the repository's TODO urgency semantics.
...
For docs-only Agent Note work, run at least the documentation-sync check, lint, and `git diff --check`. For code comments or skill changes, also run the relevant validator when one exists. Select any other evidence from the outgoing diff; a pre-push hook, where present, contributes typecheck only.
