...
## This Machine: WSL2 → Windows Edge (headless-first)

This box is pi inside WSL2; the only Chromium browser is Microsoft Edge on
Windows (host). Chrome is NOT installed here.

### Priority order (each step only if the previous one is insufficient)

1. **curl / fetch tool** — plain public pages and APIs.
2. **Headless one-shot screenshot** — screenshot / vision tasks on public
   pages. Nothing left running:
   `~/.agents/skills/browser-harness/scripts/bh-headless shot <url> <out.png> [WxH]`
   (or `headless_shot(url, path)` from agent_helpers.py), then feed the PNG
   to the vision tool. Full-page / element clips: use interactive headless
   (step 3) with `capture_screenshot(full=True)`.
3. **Headless interactive session** — clicking / typing / JS on pages
   without bot protection:
   `echo 'print(page_info())' | ~/.agents/skills/browser-harness/scripts/bh-headless`
   Fresh temp profile on :9335, `BU_CDP_URL` set; the instance is
   force-killed and its profile deleted on exit — zero leftover tabs,
   guaranteed by mechanism, not discipline.
4. **Headed dedicated Edge** — ONLY for: user's logged-in session needed,
   bot-protected pages / captcha likely, or headless failed. Zero popups.

### Headed mode: cleanup is MANDATORY (硬性规则)

After EVERY headed task: close your own tabs, then run
`~/.agents/skills/browser-harness/scripts/bh-edge-close all` (or
`wsl_edge_cleanup()` in agent_helpers.py). It kills the dedicated instance
and wipes session-restore state so tabs never resurrect. Login state
survives (cookies live in the profile). NEVER leave the dedicated Edge
running with task tabs — a fresh launch must show one newtab only.

### Headed connection modes (fallback, when step 4 applies)

- **Default: dedicated automation Edge** (port 9333, own profile) — ZERO
  popups. `export BU_CDP_URL=http://localhost:9333`, or `~/.agents/skills/browser-harness/scripts/bh-edge`
  (auto-launches the instance), or `wsl_edge_connect()` /
  `wsl_edge_auto()` from `agent_workspace/agent_helpers.py`.
- **Fallback: user's daily Edge** (login state) — connect via `BU_CDP_WS`
  read from `/mnt/c/Users/yceachan/AppData/Local/Microsoft/Edge/User Data/DevToolsActivePort`
  (its HTTP API 404s /json/version; one Allow popup per Edge session).
- Full procedure + cookie-transfer notes: `references/wsl-edge.md`
  (also at `$BH_AGENT_WORKSPACE/domain-skills/wsl-windows-edge/connect.md`).
