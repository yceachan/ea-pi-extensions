...
| 失败时机 | registry 状态 | 恢复动作 | 条件 / 备注 |
| --- | --- | --- | --- |
| 本地 push 失败（脚本第 4 步） | 零发布 | 手工 `git push && git push --tags` | commit + tag 已在本地生成，脚本只是没推出去 |
| CI 检测/环境步失败 | 零发布 | **tag 重写**：删 tag → 修好后重推同名 tag（workflow 重新触发） | 先 `npm view <pkg>@<ver>` 确认零发布；无外部消费者；正常发布后禁用 |
| CI 发布步部分失败 | 部分包已发 | **幂等 rerun**（`gh run rerun`）：publish 循环逐个跳过已发布版本 | 版本已消费，**禁止 tag 重写** |
| 全部发布成功 | 全部 | 无动作 | — |

容灾设计的三个支柱（与 mono-release 守卫互补）：

- **幂等 publish 循环**：`npm view <name>@<ver>` 存在即跳过——rerun 安全，部分失败不再是事故
- **空发布守卫**：失败后 "bump 前进" 是死路（新 release 的 diff 为空被拦截），强制走正确的恢复路径
- **版本前置守卫**：目标版本已在 registry 存在即中止——发布前拦截撞版本

bump 前进仅在一种情况下可行：失败后有**新的包内容变更**落地，下次 release 的 diff 非空。
...
| CI 发布部分失败 | **rerun workflow 优先**（publish 循环幂等：已发布版本自动跳过，见下方容灾矩阵）；手动补发失败包（`npm publish --workspace=@yceachan/<pkg>`）兜底。禁止 bump 前进——新 release 的 diff 为空，会被空发布守卫拦截 |
| CI 检测/环境步失败（零发布） | tag 重写：删 tag → 修好后重推同名 tag（workflow 重新触发）。**安全条件：`npm view` 确认该版本零发布、且无外部消费者抓过旧 tag**（单人仓库成立；正常发布后 tag 永不改写） |
...
| 本地领先 registry | 上次发布未完成（如 CI 失败）；按容灾矩阵处理，`--sync` 会拒绝降级 |
