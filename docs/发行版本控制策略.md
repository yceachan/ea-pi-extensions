...
sequenceDiagram
  ...
  Dev->>Local: 手工提交 changelog/pi-gadget/v0.3.0/log.md（docs(changelog):）
...
| 环节 | 工具 | 职责 |
| --- | --- | --- |
| 版本编辑 | `mono-sync.mjs` | 逐包检查 / 对齐 registry 基线（纯文件操作，不碰 git） |
...
./gbump -p pi-gadget -c --minor                  # 创建 changelog 骨架并打印路径
```
...
预检通过后委托 mono-release 执行发版仪式；`-c` 只创建 changelog 骨架并打印路径
（不检查工作区、不发版），填写完成后按提示走 gcm 提交。
...
| 本地落后 registry | `mono-sync -- --sync` 把落后包对齐到各自基线，再走 release |
