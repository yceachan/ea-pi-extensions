...
│   ├── pi-gadget/
│   │   └── vX.Y.Z/
│   │       └── log.md               # 逐包发布说明（手工提交；示例——首个逐包 changelog 随下次发版产生）
│   └── ...
├── scripts/
│   ├── mono-release.mjs                  # 发版：bump + release 提交 + 逐包 tag + push
│   └── mono-tagcheck.mjs                 # 逐包 version 号检查/对齐/复位（只动版本号，不做版本控制）
├── .github/workflows/
...
| changelog 未写 | 硬失败：缺失或空骨架即中止。先 `./gcm -c` 骨架 → 填写 → `docs(changelog): <pkg> vX.Y.Z` 提交。CI 侧 Release 页在 notes 缺失时仍回退 `--generate-notes`——那是手工 tag 的最后防线，正常发版路径已被本地硬守卫拦截 |
