...
async function resolveScope(input) {
	const candidates = [...packageScopes(), ...CROSS_SCOPES];
	if (candidates.includes(input)) return input; // 精确命中: 包名或跨切面词表

	const scored = candidates
		.map((s) => ({ scope: s, score: fuzzyScore(input, s) }))
	...
	if (scored.length === 0) {
		fail(
			`scope "${input}" 无候选匹配——包名: ${packageScopes().join(", ")}；跨切面: ${CROSS_SCOPES.join("/")}`,
		);
	}
	...
	if (!interactive) {
		if (scored.length === 1) {
			if (!opts.yes) {
				fail(
					`模糊匹配 "${input}" → ${scored[0].scope}（二次确认: 追加 -y 接受）`,
				);
			}
			warn(`模糊匹配 "${input}" → ${scored[0].scope}（-y 已确认）`);
			return scored[0].scope;
		}
		fail(
			`模糊匹配 "${input}" 命中多个候选（${scored.map((x) => x.scope).join(" / ")}）——请使用完整包名，或 TTY 下交互选择`,
		);
	}

	if (scored.length === 1) {
		const scope = scored[0].scope;
		const ans = await ask(
			`模糊匹配 "${input}" → 候选 ${scope}。确认使用? [Y/n] `,
		);
		if (ans === "" || /^y(es)?$/i.test(ans)) return scope;
		fail("已取消");
	}

	console.warn(`⚠ 模糊匹配 "${input}" 命中多个候选:`);
	scored.forEach((x, i) => console.log(`  ${i + 1}) ${x.scope}`));
	const ans = await ask("输入序号或完整 scope 名确认（回车取消）: ");
	if (/^\d+$/.test(ans)) {
		const pick = scored[Number(ans) - 1];
		if (pick) return pick.scope;
		fail(`无效序号: ${ans}`);
	}
	if (candidates.includes(ans)) return ans;
	fail("已取消");
}
...
function validateSubject(subject) {
	...
	if (
		/[\u{1F000}-\u{1FAFF}\u{2600}-\u{27BF}\u{2B00}-\u{2BFF}\u{FE0F}]/u.test(
			subject,
		)
	) {
...
async function main() {
	const scope =
		opts.scope !== undefined ? await resolveScope(opts.scope) : undefined;
